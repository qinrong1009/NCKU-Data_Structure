...
struct Node* mergeCircularLists(struct Node* list1, struct Node* list2) {
    if (!list1) return list2;
    if (!list2) return list1;

    struct Node *head1 = list1, *head2 = list2;
    struct Node *tail1 = list1->left, *tail2 = list2->left;
    ...
    // 確認尾部並重建循環結構
    struct Node* mergedHead = dummy.right;
...
// 輸出循環列表
void printCircularList(struct Node* list) {
    if (!list) return;
...
int main() {
    ...
    // 合併兩個列表
    struct Node* mergedList = mergeCircularLists(list1, list2);
